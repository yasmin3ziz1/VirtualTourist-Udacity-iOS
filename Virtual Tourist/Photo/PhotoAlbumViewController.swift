...
class PhotoAlbumViewController: UIViewController, MKMapViewDelegate {
    ...
    private func fetchPhotosFromAPI(_ pin: Pin) {
        ...
        self.updateStatusLabel("Fetching all images")
        ...
        Client.shared().searchBy(latitude: lat, longitude: lon, totalPages: totalPages) { (photosParsed, error) in
            ...
            if let photosParsed = photosParsed {
                ...
                if totalPhotos == 0 {
                    self.updateStatusLabel("This pin has no images.")
                }
            } else if let error = error {
                print("\(#function) error:\(error)")
                self.showInfo(withTitle: "Error", withMessage: error.localizedDescription)
                self.updateStatusLabel("Error, please try again.")
            }
        }
    }
    ...
    func updateBottomButton() {
        // Here to Refresh image collection
